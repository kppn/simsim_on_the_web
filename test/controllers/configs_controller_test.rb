...
class ConfigsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create config" do
    assert_difference('Config.count') do
      post configs_url, params: { config: { log_title: @config.log_title, name: @config.name } }
    end
  ...
  test "should update config" do
    patch config_url(@config), params: { config: { log_title: @config.log_title, name: @config.name } }
